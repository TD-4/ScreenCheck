...
def resnet50_backbone(lda_out_channels, in_chn, pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model_dict['conv1.weight'] = save_model['conv1.weight'].data.repeat(1,10,1,1)
        state_dict = {k: v for k, v in save_model.items() if k in model_dict.keys() and k not in ('conv1.weight') }
